...
function initTeamCarousel() {
  const gallery = document.querySelector('#team-gallery .gallery');
  if (!gallery) return;
  ...
  prevBtn.id = 'team-prev';
  prevBtn.className = 'team-nav';
  prevBtn.setAttribute('aria-label', 'Previous team member');
  prevBtn.innerHTML = '‹';

  const nextBtn = document.createElement('button');
  nextBtn.id = 'team-next';
  nextBtn.className = 'team-nav';
  nextBtn.setAttribute('aria-label', 'Next team member');
  nextBtn.innerHTML = '›';

  // Insert them around the gallery
  gallery.parentNode.style.position = 'relative';
  gallery.parentNode.insertBefore(prevBtn, gallery);
  gallery.parentNode.appendChild(nextBtn);

  function updateView() {
    // Determine how many to show
    const isMobile = window.matchMedia(`(max-width: ${getComputedStyle(document.documentElement)
                            .getPropertyValue('--bp-sm')})`).matches;
    const visibleCount = isMobile ? 1 : 2;

    // Show/hide items
    items.forEach((fig, i) => {
      fig.style.display = (i >= currentIndex && i < currentIndex + visibleCount)
        ? 'block'
        : 'none';
    });

    // Disable at ends (optional wrap-around logic could go here)
  ...
  nextBtn.addEventListener('click', () => {
    const isMobile = window.matchMedia(`(max-width: ${getComputedStyle(document.documentElement)
                            .getPropertyValue('--bp-sm')})`).matches;
    const maxIndex = items.length - (isMobile ? 1 : 2);
